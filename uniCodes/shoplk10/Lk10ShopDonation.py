...
DONATION_FILE = 'donations_total.txt'
...
class Pedido:
    ...
    def gerar_nota_fiscal(self, client_name, donation=0):
        nota = "NOTA FISCAL\n"
        nota += "--------------------------------\n"
        ...
        nota += "--------------------------------\n"
        if donation > 0:
            nota += f"Donation: R${donation:.2f}\n"
            total += donation
        nota += f"Total: R${total:.2f}\n"
        
        purchase_details = [{"produto": produto.nome, "preco": produto.preco, "quantidade": quantidade} for produto, quantidade in self.produtos]

        logging.info(f"{client_name} has made a purchase: {purchase_details} at {formatted_date} with [ donation: R${donation:.2f} ]")
...
def get_donation_amount(stdscr):
    stdscr.clear()
    stdscr.addstr(0, 0, "Do you want to contribute to helping the state of Rio Grande Do Sul")
    stdscr.addstr(1, 0, "in Brazil suffering with a lot of floods?")
    stdscr.addstr(2, 0, "(Any value above 0.009 cents is accepted)")
    stdscr.addstr(3, 0, "")  #Newline;
    stdscr.addstr(4, 0, "Enter donation amount: ")
    stdscr.refresh()
    curses.echo()
    donation = float(stdscr.getstr(4, len("Enter donation amount: "), 10).decode('utf-8'))
    curses.noecho()
    return donation


def update_donation_total(amount):
    total = 0.00
...
def main_interaction(stdscr, cliente):
    ...
    donation = get_donation_amount(stdscr)
    if donation >= 0.009:
        update_donation_total(donation)
        nota_fiscal = cliente.pedido.gerar_nota_fiscal(cliente.nome, donation)
    else:
    ...
    logging.info(f"A purchase has been completed by {cliente.nome} [SUCCESS]")
